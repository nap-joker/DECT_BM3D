...
class Solver_CT(object):   #CT图像重建任务的求解器
    ...
    def train(self):
        ...
        for epoch_i in range(self.start_epoch + 1, self.end_epoch + 1):
            ...
            step = 0
            for i, data in enumerate(self.train_loader): #500
                step = step + 1

                Truth_A_T, Truth_F_T, Truth_C_T = data   #[1, 512, 512]
                Truth_A_T = torch.unsqueeze(Truth_A_T, 1).cpu().data.numpy() #(1, 1, 512, 512)
                ...
                # numpy to tensor
                Truth_A_T = torch.from_numpy(Truth_A_T).to(self.device) #[1, 1, 512, 512]
                Truth_F_T = torch.from_numpy(Truth_F_T).to(self.device)
                Truth_C_T = torch.from_numpy(Truth_C_T).to(self.device)

                # creat low/high FBP
                #primal_op_layer:OperatorModule(RayTransform) ((512, 512) -> (64, 1024))
                #dual_op_layer:OperatorModule(OperatorComp) ((64, 1024) -> (512, 512))
                ...
                batch_x = torch.cat((Truth_A_T, Truth_F_T, Truth_C_T), 1).to(self.device) #[1, 3, 512, 512]
                # predict
